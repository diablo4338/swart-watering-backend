...
- `devices.name` is the backend-owned display name. MCU-reported state, including `device.name`, belongs exclusively to the persisted snapshot plus `DeviceRuntimeState`; snapshots replace the base runtime state and confirmed callbacks apply runtime patches. Never copy MCU state into `devices`, never project MCU fields from registry records, and never use `devices.name` as a fallback for MCU-reported values. Device cards combine backend metadata from the registry with MCU state from the current runtime projection.
- A successful discovery response must be persisted as the registered device's first snapshot. Subsequent card projections must recover from that snapshot and then apply confirmed callback patches; discovery operation payloads are not a device-state store.
