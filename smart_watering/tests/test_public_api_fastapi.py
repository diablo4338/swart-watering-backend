...
def test_android_release_returns_not_found_before_first_publication() -> None:
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert response.json()["error"] == {
            ...
            "title": "App release is not available",
            "detail": "no Android release has been published",
            "retryable": False,
        }
...
def test_fastapi_health_and_auth_contract() -> None:
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert unauthorized.json()["error"]["title"] == "Sign-in is required"
...
def test_v3_device_card_exposes_server_driven_blocks_and_actions() -> None:
    with tempfile.TemporaryDirectory() as temp_dir:
        client, cli = make_client(temp_dir)
        cli.auth.add_user("client", "secret-password")
        registered = cli.registry.add("10.0.0.1", "plant", "avocado")
        ...
        with (
            ...
            card = client.get(f"/api/v3/devices/{device_id}/card", headers=headers)
        ...
        assert devices.json()["devices"] == [{
            ...
            "card_href": f"/api/v3/devices/{device_id}/card",
        }]
        ...
        assert refresh_action["id"] == "refresh_card"
        assert refresh_action["request"] == {
            "method": "POST",
            "href": f"/api/v3/devices/{device_id}/actions/refresh-card",
            "body": {"binding": "none"},
        }
        ...
        with patch.object(
            ...
            fast_queue = client.get(
                f"/api/v3/devices/{device_id}/card/blocks/operation_queue",
                headers=headers,
            )
        ...
        with patch.object(
            ...
            for snapshot_block in ("overview", "control", "watering_parameters"):
                isolated = client.get(
                    f"/api/v3/devices/{device_id}/card/blocks/{snapshot_block}",
                    headers=headers,
                )
        ...
        with (
            ...
            history = client.get(
                f"/api/v3/devices/{device_id}/card/blocks/watering_history",
                headers=headers,
            )
        ...
        overview = client.get(
            f"/api/v3/devices/{device_id}/card/blocks/overview",
            headers=headers,
        )
        ...
        action = client.post(
            f"/api/v3/devices/{device_id}/actions/set-sleep",
            json={"enabled": True},
            headers=headers,
        )
        ...
        assert cli.operations.latest_non_terminal(device_id, "sleep_enable") is not None
        interval_action = client.post(
            f"/api/v3/devices/{device_id}/actions/set-sleep-interval",
            json={"minutes": 17},
            headers=headers,
        )
        ...
        remaining = cli.operations.details_from_operations(
            cli.operations.list_non_terminal(device_id)
        )
...
def test_v3_device_resource_id_survives_backend_rename() -> None:
    with tempfile.TemporaryDirectory() as temp_dir:
        client, cli = make_client(temp_dir)
        cli.auth.add_user("client", "secret-password")
        device = cli.registry.add("10.0.0.2", "plant", "old-name")
        token = client.post(
            "/api/v3/auth/login",
            json={"username": "client", "password": "secret-password"},
        ).json()["token"]
        ...
        renamed = client.post(
            f"/api/v3/devices/{device.id}/actions/rename-backend",
            json={"name": "new-name"},
            headers=headers,
        )

        assert renamed.status_code == 200
        assert renamed.json()["card"]["device_id"] == device.id
        overview = next(
            block for block in renamed.json()["card"]["blocks"]
            if block["id"] == "overview"
        )
        assert overview["data"]["title"] == "new-name"
        assert client.get(
            f"/api/v3/devices/{device.id}/card", headers=headers
        ).status_code == 200
        assert client.get(
            "/api/v3/devices/old-name/card", headers=headers
        ).status_code == 400
        assert client.get(
