...
def test_presence_monitor_updates_runtime_state_from_health_probe() -> None:
    device = SimpleNamespace(id="device-id", name="avocado", base_url="http://10.0.0.1")
    presence = DevicePresenceRegistry()
    assert presence.get(device.id).state == "offline"
    ...
    async def scenario() -> None:
        ...
        assert presence.get(device.id).online is True
